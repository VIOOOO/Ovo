...
export default {
  ...
  methods: {
    toggleSheets(event) {
      this.$refs.toggleicon.style.transform = this.showSheets
        ? "rotate(-90deg) translate3d(50%, 0, 0)"
  ...
  mounted() {
    this.data_item = this.itemqqq;
    this.data_index = this.indexqqq;
    // console.log(JSON.stringify(this.$store.getters.getMusicAllList.sheets))
    // console.log(JSON.stringify(this.$store.state.commit))
  }
};
</script>
