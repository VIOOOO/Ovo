import 'dart:convert';

import 'package:flutter/material.dart';

import 'package:dio/dio.dart';

// 新闻 下拉刷新 上拉加载更多
class NewsPage extends StatefulWidget {
  NewsPage({Key key}) : super(key: key);

  @override
  _NewsPageState createState() => _NewsPageState();
}

class _NewsPageState extends State<NewsPage> {
  List _list = [];

  // 页码
  int _page = 1;

  // 判读有没有数据
  bool hasMore = true;

  // 定义滑动事件
  ScrollController _scrollController = new ScrollController();

  @override
  void initState() {
    super.initState();
    this._getData();

    // 监听滚动条事件
    _scrollController.addListener(() {
      // // 获取滚动条下拉的距离
      // print(_scrollController.position.pixels);
      // // 获取整个页面的高度
      // print(_scrollController.position.maxScrollExtent);

      // 滚动距离大于页面减去的高度时候，触发事件
      if (_scrollController.position.pixels >
          _scrollController.position.maxScrollExtent - 40) {
        this._getData();
      }
    });
  }

  void _getData() async {
    if (this.hasMore) {
      var apiUrl =
          "http://www.phonegap100.com/appapi.php?a=getPortalList&catid=20&page=${this._page}";
      Response response = await Dio().get(apiUrl);
      var res = json.decode(response.data)['result'];
      setState(() {
        // 拼接数据
        this._list.addAll(res);
        // 保存页面并加1
        this._page++;
      });
      // 判读是否最后一页，当加载页面数据比较少时，就不再可以继续上拉加载页面
      if (res.length < 20) {
        setState(() {
          this.hasMore = false;
        });
      }
      // print(this._list);
    }
  }

  // 下拉刷新 RefreshIndicator onRefresh 返回 Future 类型
  Future<void> _onRefresh() async {
    // 延时
    await Future.delayed(Duration(milliseconds: 2000), () {
      print("请求结束");
      this._getData();
    });
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("新闻列表"),
      ),
      // list.map() 或者  ListView.builder 里可以判断
      body: this._list.length > 0
          ? RefreshIndicator(
              onRefresh: _onRefresh,
              child: ListView.builder(
                controller: _scrollController,
                itemCount: this._list.length,
                itemBuilder: (context, index) {
                  if (index == this._list.length - 1) {
                    // 如果页面渲染拉到底部，调用 loading 组件
                    return Column(
                      children: <Widget>[
                        // 标题内容最大1行
                        ListTile(
                          title: Text(
                            "${this._list[index]['title']}",
                            maxLines: 1,
                          ),
                          onTap: () {
                            // 跳转到新闻详情 并传递id
                            Navigator.pushNamed(context, '/newsContent',
                                arguments: {
                                  "aid": this._list[index]['aid'],
                                });
                          },
                        ),
                        Divider(),
                        // 调用 loading 组件
                        _getMoreWidget(),
                      ],
                    );
                  } else {
                    return Column(
                      children: <Widget>[
                        // 标题内容最大1行
                        ListTile(
                          title: Text(
                            "${this._list[index]['title']}",
                            maxLines: 1,
                          ),
                          onTap: () {
                            // 跳转到新闻详情 并传递id
                            Navigator.pushNamed(context, '/newsContent',
                                arguments: {
                                  "aid": this._list[index]['aid'],
                                });
                          },
                        ),
                        Divider(),
                      ],
                    );
                  }
                },
              ),
            )
          : _getMoreWidget(),
    );
  }

  // loading 组件
  Widget _getMoreWidget() {
    if (this.hasMore) {
      return Center(
        child: Padding(
          padding: EdgeInsets.all(10.0),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            crossAxisAlignment: CrossAxisAlignment.center,
            children: <Widget>[
              Text("加载中... "),
              // loading
              CircularProgressIndicator(
                strokeWidth: 1.0,
              ),
            ],
          ),
        ),
      );
    } else {
      // 拉到底部添加提示
      return Center(
        child: Text('---- 我是有底线的 ----'),
      );
    }
  }
}
